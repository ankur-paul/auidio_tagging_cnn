# Walkthrough: Pipeline Performance & Correctness Fixes

## Changes Made

### 1. [main.py](file:///home/ankur/projects/audioset_tagging_cnn/pytorch/main.py) — Critical Fix
Removed `choices=['balanced_train', 'full_train']` restriction on `--data_type` so `combined_train` (used in walkthrough2.md Step 7) is accepted.
...
### 2. [convert_hf_to_wav.py](file:///home/ankur/projects/audioset_tagging_cnn/scripts/convert_hf_to_wav.py) — Multiprocessing
- Added `multiprocessing.Pool` with `--num_workers` flag (defaults to CPU count = 8)
- Extracted per-clip work into [_process_single_clip()](file:///home/ankur/projects/audioset_tagging_cnn/scripts/prepare_external_datasets.py#129-152) worker
- Falls back to single-process when `--num_workers 1`
- ~42,000 clips now processed across all 8 cores

---

### 3. [prepare_external_datasets.py](file:///home/ankur/projects/audioset_tagging_cnn/scripts/prepare_external_datasets.py) — Multiprocessing + FSD50K Fix
- **Multiprocessing:** Shared [_process_single_clip()](file:///home/ankur/projects/audioset_tagging_cnn/scripts/prepare_external_datasets.py#129-152) worker + [_run_parallel()](file:///home/ankur/projects/audioset_tagging_cnn/scripts/prepare_external_datasets.py#154-186) orchestrator, `--num_workers` on all sub-commands
- **FSD50K loader fix:** Removed dead JSON code path ([_load_fsd50k_ground_truth](file:///home/ankur/projects/audioset_tagging_cnn/scripts/prepare_external_datasets.py#415-446) lines 396-419 with `pass` fallthrough). Now reads CSV directly from [FSD50K.ground_truth/dev.csv](file:///home/ankur/projects/audioset_tagging_cnn/raw/fsd50k/FSD50K.ground_truth/dev.csv)
- Pre-filters work items (metadata errors) before parallel loop for cleaner error reporting

---

## Verification
- All 3 files pass Python AST syntax validation
- `--help` flags tested (slow import times prevented full output capture in CI, but structurally correct)
- FSD50K ground truth CSV format confirmed: `fname,labels,mids,split` with quoted comma-separated values — `csv.DictReader` handles this correctly
