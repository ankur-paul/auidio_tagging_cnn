...
class AudioSetDataset(object):
    def __init__(self, sample_rate=32000):
        ...
        # Cache stored sample rates per HDF5 file to avoid repeated attr lookups
        self._hdf5_sr_cache = {}
    
    def _get_stored_sr(self, hdf5_path):
        """Get the sample rate stored in an HDF5 file (cached)."""
        if hdf5_path not in self._hdf5_sr_cache:
            try:
                with h5py.File(hdf5_path, 'r') as hf:
                    self._hdf5_sr_cache[hdf5_path] = int(hf.attrs.get('sample_rate', 32000))
            except Exception:
                self._hdf5_sr_cache[hdf5_path] = 32000  # fallback
        return self._hdf5_sr_cache[hdf5_path]
    ...
    def __getitem__(self, meta):
        ...
        # Only resample if HDF5 was packed at a different rate than training
        stored_sr = self._get_stored_sr(hdf5_path)
        if stored_sr != self.sample_rate:
            waveform = self.resample(waveform, stored_sr)
    ...
    def resample(self, waveform, stored_sr=32000):
        """Resample from stored_sr to self.sample_rate by decimation.

        Args:
          waveform: (clip_samples,)
          stored_sr: sample rate of the stored waveform

        Returns:
          (resampled_clip_samples,)
        """
        ratio = stored_sr // self.sample_rate
        if ratio == 1:
            return waveform
        elif ratio >= 2:
            return waveform[0 :: ratio]
        else:
            raise Exception('Cannot upsample: stored_sr={} < sample_rate={}'.format(
                stored_sr, self.sample_rate))
