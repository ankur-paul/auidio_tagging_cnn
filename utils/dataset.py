...
# ---------------------------------------------------------------------------
def _resample_poly(audio, orig_sr, target_sr):
    """High-quality polyphase resampling via scipy."""
    from scipy.signal import resample_poly as _resample_poly_impl
    g = gcd(orig_sr, target_sr)
    up = target_sr // g
    dn = orig_sr // g
    return _resample_poly_impl(audio.astype(np.float64), up, dn).astype(np.float32)


def _load_audio_worker(args_tuple):
    """Worker: load a single WAV, resample if needed, pad/truncate, convert to int16.

    Args:
        args_tuple: (index, audio_path, target_sr, clip_samples)

    Returns:
        (index, waveform_int16) on success, (index, None) on failure.
    """
    idx, audio_path, target_sr, clip_samples = args_tuple
    ...
    try:
        ...
        if sr != target_sr:
            audio = _resample_poly(audio, sr, target_sr)

        # Pad or truncate to exact clip length
        audio = pad_or_truncate(audio, clip_samples)

        # Convert to int16 for compact storage
        return (idx, float32_to_int16(audio))
...
def pack_waveforms_to_hdf5(args):
    ...
    Optimized version: uses multiprocessing for parallel audio loading and
    soundfile for fast WAV reading. Resampling is only done when the source
    sample rate differs from the target.
    """
    ...
    # Use CLI sample rate, falling back to config
    ...
    logging.info('Packing at sample_rate={}, clip_samples={}, num_workers={}'.format(
        sample_rate, clip_samples, num_workers))
    ...
    # Build list of work items (only for files that exist)
    work_items = []
    missing_indices = []
    for n in range(audios_num):
        audio_path = os.path.join(audios_dir, meta_dict['audio_name'][n])
        if os.path.isfile(audio_path):
            work_items.append((n, audio_path, sample_rate, clip_samples))
        else:
            missing_indices.append(n)
            logging.info('{} File does not exist! {}'.format(n, audio_path))

    logging.info('{} files found, {} missing'.format(len(work_items), len(missing_indices)))
    ...
    with h5py.File(waveforms_hdf5_path, 'w') as hf:
        ...
        if num_workers <= 1:
            ...
            for item in work_items:
                idx, waveform = _load_audio_worker(item)
                hf['audio_name'][idx] = meta_dict['audio_name'][idx].encode()
                if waveform is not None:
                    hf['waveform'][idx] = waveform
                else:
                    logging.warning('{} Failed to load audio'.format(idx))
                hf['target'][idx] = meta_dict['target'][idx]
                processed += 1
                if processed % 500 == 0 or processed == total:
                    logging.info('  {}/{} ({:.1f}%)'.format(processed, total,
                        100.0 * processed / total))
        else:
            # Multi-process path: parallel audio loading, sequential HDF5 writes
            with Pool(processes=num_workers) as pool:
                for idx, waveform in pool.imap_unordered(
                    _load_audio_worker, work_items, chunksize=64
                ):
                    hf['audio_name'][idx] = meta_dict['audio_name'][idx].encode()
                    if waveform is not None:
                        hf['waveform'][idx] = waveform
    ...
    logging.info('Pack hdf5 time: {:.3f} s'.format(time.time() - total_time))
...
if __name__ == '__main__':
    ...
    parser_pack_wavs.add_argument('--sample_rate', type=int, default=16000, help='Target sample rate for packing (default: 16000).')
    parser_pack_wavs.add_argument('--num_workers', type=int, default=max(1, cpu_count()), help='Number of parallel workers (default: CPU count).')
